...
Defines echo's provider/adapter architecture — the `Api` × `Provider` factoring, the open adapter registry, v1's supported provider set, per-provider compatibility flags, streaming mapping rules, tool mapping, Anthropic prompt-caching behaviour, and credential resolution order. Wire-protocol details live here; the caller-facing types are in `echo-client-api`.
