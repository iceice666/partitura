# Spec: echo-client-api

## Purpose

Defines the public Rust API surface of the `echo` library crate — the types callers use to build requests (`Context`, `Message`, content blocks, `Options`) and the streaming interface they consume (`stream`, `complete`, event union). This spec governs what `voice` and other consumers see; it does not cover wire-level adapter details (see `echo-providers`).
