# voice-failure-contract

## Purpose

Governs how Voice handles unrecoverable failure conditions: the summarize-state routine
(compact vs. handoff), digest-mode selection by failure type, in-loop compaction triggers,
`compact` built-in semantics, terminal failure branches, and the budget-exhaustion handoff.
