...
Governs the `score.run-report/v1` JSON document that Voice writes to `VOICE_REPORT_PATH` on
exit: when it is written, what fields are required, and the shapes of outcome-specific payloads.
...
field is defined in `score.run-report/v1` in `CONTRACT.md` and `voice/spec/report.md`.
