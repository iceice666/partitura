...
Defines how echo resolves configuration and credentials at runtime — file locations, permissions, environment variables, and the security boundary between echo-owned runtime state (OAuth tokens) and operator-managed secrets (API keys via environment or sops-nix). In-process decryption and a `secret-edit` command are explicitly deferred to a future change.
