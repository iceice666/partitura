...
Governs the native per-process agent loop inside `voice`: how Voice assembles the context,
drives the `echo` library, emits progress events, executes tool calls, handles exit signals,
enforces budgets, and terminates.
